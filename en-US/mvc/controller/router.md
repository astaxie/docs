...
## RESTFul router
Let's talk about RESTFul first. RESTFult is a popular way in API development. Beego supports it implict :TODO. Executing GET method for GET request and POST method for POST request. The defaut router is RESTFul.
...
方法名在内部是保存了用户设置的. :TODO Url will match by lowercase, so `object/LOGIN` will also map to `Login` method
